...
#
# fn_netcdf:
# Name (not including path) of the netcdf file generated by the external
# model.  Currently used for WRF-based RAP and HRRR external model data.
#
# input_type:
...
"RAP")
  if [ "${WRF_FILE_FMT_ICS}" = "grib2" ]; then
    input_type="grib2"
    external_model="RAP"
    fn_grib2="${EXTRN_MDL_FNS[0]}"
  elif [ "${WRF_FILE_FMT_ICS}" = "netcdf" ]; then
    input_type="wrf"
    external_model="WRF"
    fn_netcdf="${EXTRN_MDL_FNS[0]}"
  fi
  ;;
